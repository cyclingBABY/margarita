...
import { Calendar as CalendarIcon, MapPin, Plus, Check, X, Clock, User, AlertCircle } from 'lucide-react';
...
export const AdminEventsView = () => {
  ...
  const [activeFilter, setActiveFilter] = useState<'all' | 'pending' | 'scheduled' | 'cancelled'>('all');
  ...
  const handleUpdateStatus = async (id: number, status: 'scheduled' | 'cancelled') => {
    try {
      const token = localStorage.getItem('token');
      ...
      toast.success(`Event successfully ${status === 'scheduled' ? 'approved' : 'cancelled'}.`);
      fetchEvents();
    } catch (error: any) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'pending_approval':
        return <Badge className="bg-orange-100 text-orange-800 rounded-none border-none font-bold uppercase text-[9px]">Pending Approval</Badge>;
      case 'cancelled':
  ...
  const filteredEvents = events.filter(event => {
    ...
    if (activeFilter === 'pending') return event.status === 'pending_approval';
  ...
  return (
    <div className="space-y-6">
      <div className="bg-hotel-blue p-6 text-white flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-serif text-hotel-sand">Events Management</h2>
          <p className="text-white/60 text-sm">Schedule hotel events or manage guest private gathering requests</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[500px] rounded-none border-hotel-blue/10 bg-white">
            <DialogHeader className="border-b border-hotel-blue/5 pb-4">
              <DialogTitle className="text-xl font-serif text-hotel-blue">Schedule New Event</DialogTitle>
              <DialogDescription className="text-xs">Create and publish an event to the hotel calendar.</DialogDescription>
            </DialogHeader>
            <form onSubmit={handleScheduleEvent} className="space-y-4 pt-4">
              <div className="space-y-2">
                <Label htmlFor="title" className="text-[10px] uppercase font-bold text-hotel-blue tracking-widest">Event Title *</Label>
                <Input 
                  id="title"
                  required
                  placeholder="e.g. Wine Tasting Session, Jazz Night"
              ...
              <div className="space-y-2">
                ...
                <textarea 
                  id="description"
                  placeholder="Provide event details, itinerary, or booking information..."
      ...
      <div className="flex gap-2 border-b border-hotel-blue/5 pb-4">
        {(['all', 'pending', 'scheduled', 'cancelled'] as const).map((filter) => (
          <Button
            key={filter}
            onClick={() => setActiveFilter(filter)}
            variant={activeFilter === filter ? 'default' : 'outline'}
            className={`rounded-none text-[10px] uppercase font-bold tracking-wider px-4 h-9 ${
              activeFilter === filter
                ? 'bg-hotel-blue text-white hover:bg-hotel-blue'
                : 'border-hotel-blue/10 text-hotel-blue hover:bg-hotel-sand/50'
            }`}
          >
            {filter === 'pending' ? 'Pending Approval' : filter}
          </Button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          <div className="col-span-full text-center py-12 text-slate-400 font-medium italic">No events found matching this status.</div>
        ) : (
          filteredEvents.map((event) => (
            <Card key={event.id} className="border-none shadow-sm flex flex-col justify-between overflow-hidden group">
              ...
              {event.status === 'pending_approval' && (
                <div className="p-4 bg-hotel-sand/10 border-t border-hotel-blue/5 flex gap-2">
                  <Button 
                    onClick={() => handleUpdateStatus(event.id, 'scheduled')}
                    className="flex-1 bg-green-600 hover:bg-green-700 text-white rounded-none font-bold uppercase text-[10px] tracking-wider h-9"
                  >
                  ...
                  <Button 
                    ...
                    variant="destructive"
                    className="flex-1 rounded-none font-bold uppercase text-[10px] tracking-wider h-9"
                  >
              ...
              {event.status === 'scheduled' && (
                <div className="p-4 bg-hotel-sand/10 border-t border-hotel-blue/5 flex">
                  <Button 
                    onClick={() => handleUpdateStatus(event.id, 'cancelled')}
                    variant="outline"
                    className="w-full border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700 rounded-none font-bold uppercase text-[10px] tracking-wider h-9"
                  >
                    <X className="h-4 w-4 mr-1" /> Cancel Event
                  </Button>
                </div>
              )}
            </Card>
